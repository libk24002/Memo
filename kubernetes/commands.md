...
# Patch Service
kubectl patch svc my-svc -p '{"spec":{"ports":[{"port":8080,"targetPort":80}]}}'
```
...
kubectl label pods <pod-name> env=prod --overwrite  # 覆盖已有标签
...
```bash
# 创建 ResourceQuota
kubectl create quota my-quota --hard=pods=10,cpu=4,memory=8Gi -n <namespace>
...
kubectl get pods --all-namespaces --field-selector=status.phase=Failed | \
  awk '{if ($4 == "Evicted") print "kubectl delete pod " $2 " -n " $1}' | \
  bash
...
echo "All CrashLoopBackOff pods have been restarted."
```
