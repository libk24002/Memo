...
* role
  + master: `rock-01`, `rock-02`, `rock-03`
  + worker: `rock-04` ~ `rock-05` ~ `rock-06`

### 安装初始化
1. 部署配置概述
    * 选用 `rock-01` 作为 `Ansible 主控机`
    * 选用 `rock-01` ~ `rock-03` 作为`集群master节点`
    * 确保 `Ansible 主控机` 与 `其他节点`已配置SSH免密登录
    * 确保 `Ansible 主控机` 已经配置可用mirror源
2. 配置 `rock-01` ansible 环境
    * ```

1. 选用`rock-01`作为资源分发节点，配置`/etc/hosts`
    * 确保`rock-01`节点与其他节点已配置免密登录
    * ```shell
      cat >> /etc/hosts <<EOF
      192.168.31.11 rock-01
      192.168.31.12 rock-02
      192.168.31.13 rock-03
      192.168.31.14 rock-04
      192.168.31.15 rock-05
      192.168.31.16 rock-06
      EOF
      ```
    * ```shell
      ssh-copy-id -i ~/.ssh/id_rsa.pub root@rock-02
      ssh-copy-id -i ~/.ssh/id_rsa.pub root@rock-03
      ssh-copy-id -i ~/.ssh/id_rsa.pub root@rock-04
      ssh-copy-id -i ~/.ssh/id_rsa.pub root@rock-05
      ssh-copy-id -i ~/.ssh/id_rsa.pub root@rock-06
      ```
...
以下是在离线环境中部署 Kubernetes v1.32.5 所需的镜像列表。请确保在离线环境中导入这些镜像。

### 认证与授权组件
```bash
ghcr.io/dexidp/dex:v2.44.0                                # Dex 身份认证服务
quay.io/jetstack/cert-manager-controller:v1.18.0          # 证书管理控制器
quay.io/jetstack/cert-manager-cainjector:v1.18.0          # 证书管理注入器
```

### 网络与服务组件
```bash
registry.k8s.io/ingress-nginx/kube-webhook-certgen:v1.6.2 # Ingress Nginx 证书生成器
registry.k8s.io/ingress-nginx/controller:v1.13.2          # Ingress Nginx 控制器
```

### 数据存储组件
```bash
ecr-public.aws.com/docker/library/redis:7.2.8-alpine      # Redis 数据库
```

### 镜像导入指南

在离线环境中，您可以使用以下步骤导入上述镜像：
...
1. 在有网络连接的环境中下载镜像：
   ...
   for img in \
     ...
     quay.io/jetstack/cert-manager-cainjector:v1.18.0; do
     docker pull $img
   done
   ```
...
2. 将镜像保存为压缩文件：
   ...
   for img in \
     ...
     docker save $img -o k8s-images/$filename.tar
   done
   
   # 打包所有镜像文件
   tar -czvf k8s-v1.32.5-images.tar.gz k8s-images/
   ```
...
4. 在离线环境中加载镜像：
   ```bash
   # 解压镜像文件
   tar -xzvf k8s-v1.32.5-images.tar.gz
   
   # 加载所有镜像
   for img in k8s-images/*.tar; do
     docker load -i $img
   done
   ```
