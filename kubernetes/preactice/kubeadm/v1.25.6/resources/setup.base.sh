...
set -x
# install docker
$(if [ "8" == "$(rpm --eval '%{centos_ver}')" ]; then echo dnf; else echo yum; fi) install -y device-mapper-persistent-data lvm2 docker-ce kubelet-1.25.6-0 kubeadm-1.25.6-0 kubectl-1.25.6-0 python3 iproute-tc \
